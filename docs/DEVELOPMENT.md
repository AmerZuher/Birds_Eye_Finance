...
│  └─ ui/           29 shared, theme-driven primitives
...
- **`src/components/`** — app-specific composed components (`DebtModal`, `ExpenseModal`, `Header`, `Navbar`, ...). Its `ui/` subfolder holds **29 shared, theme-driven primitives** (CLAUDE.md rule 4) — every screen composes these; nothing hardcodes a color or duplicates another screen's pattern. Rule 4 lists all 29.
- **`src/context/`** — 12 React Contexts; see the provider-tree diagram below for what each one owns.
- **`src/db/`** — `schema.ts` (Drizzle table definitions: `expenses`, `people`, `debts`, `debtAdjustments`, `debtAttachments`, `incomeSources`, `balanceSnapshots`), `client.ts` (expo-sqlite + Drizzle setup, foreign keys on), `maintenance.ts` (post-migration people backfill and attachment reconcile), `migrations/` (drizzle-kit-generated SQL — never hand-edit, run `npm run db:generate` instead).
...
flowchart TD
    ...
    G --> BA[BalanceProvider]
    BA --> H[ChromeProvider]
    H --> R[RemindersProvider]
    R --> U[UpdatesProvider]
    U --> I[ModalPortalProvider]
    I --> J[RootLayoutInner]
```
...
- **BalanceProvider** — the balance log (`balance_snapshots`) and everything derived from it: the aged-forward current balance, each log's variance, and how stale the last one is (`src/lib/reconciliation.ts`, FEATURE_SPEC Part 9). Needs `FinanceProvider` above it for `netSavings` and `UserProvider` for the typed accounts it falls back to before the first log.
- **ChromeProvider** — header/navbar heights, the shared `blurTarget` ref, the FAB handler, whether the Log Balance sheet is open, and the "open this person/debt" request the Dashboard and a tapped reminder queue for the Debts screen.
...
- **UpdatesProvider** — the app-update setting, the daily check on launch and foreground, and the download/verify/install stages (`src/lib/updates.ts`, FEATURE_SPEC 3.6).
...
interface BackupSnapshot {
  ...
  balanceSnapshots: BalanceSnapshot[]; // the balance log; absent in files before 2.3.0
...
- **Bump the version before you build, never after.** `plugins/withReleaseApk.js` names the APK from `versionName`, so building without bumping writes over `apk/birdsEyeFinance_V<old version>.apk` with a binary that is stamped with the *old* version — and that file is one upload away from replacing a shipped release's asset. The plugin now fails the build when that file already exists; if you hit that error, bump `app.json` (rule 17) rather than deleting the file. The order that always works: bump → `prebuild` → build → tag → upload.
