Act as an elite Lead Mobile Native Engineer. Your task is to build "BirdsEye Finance", a highly polished, local-first mobile application using Expo (React Native) with Expo Router, NativeWind v4 (Tailwind), expo-sqlite via Drizzle ORM, and react-native-mmkv.

This is a multi-session project — you will not remember this conversation next time. Before anything else, write the three files below exactly as given, so every future session (including phases 2, 3, 4...) can pick up full context by reading them, without this being re-pasted. Do that now, then proceed to "Phase 1 Execution" at the end of this message.
...
=== WRITE FILE: docs/FEATURE_SPEC.md ===
...
Definition of Done:
- [ ] Boots on an EAS dev client, no red screens.
- [ ] Cold start shows platinum theme instantly, no flash.
- [ ] All 18 primitives exist, import only from theme.ts.
- [ ] Drizzle schema matches Expense/Debt/IncomeSource, working migration.
- [ ] brandIcons.ts has a working name → icon lookup (verified with Netflix/Spotify/Claude, then the test render deleted).
- [ ] Can tap through Dashboard/Expenses/Debts/Settings/About with no crash; Settings is fully real, others are placeholders.
- [ ] Android back-button chain works even with stub screens.
...
Phase 1 Execution: once the three files above exist, proceed exactly per the Phase 1 scope and Definition of Done in docs/PHASES.md. Confirm you can run the app on an EAS Development Build (not just Expo Go) before marking any Definition of Done item complete that depends on it.
